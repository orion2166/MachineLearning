 ...
 */
public class NeuralNetworkCrossValidation {
    NeuralNetwork test_node = null;
    private Vector<Vector<String[]>> used_data = new Vector<>();
    private int percent = 0;
    private Vector<double[]> result_sets = new Vector<>();
    public NeuralNetworkCrossValidation(){
        for(int i= 0;i<10;i++){
    ...
    Vector cross_validation(int numbernodes,int layers,String attribute,String data,int location,boolean binary){
        test_node = new NeuralNetwork(numbernodes,layers,attribute,data,location,binary);
        set_cross_values();
        NeuralNetwork networktest;
        for(int i = 0;i<10;i++)
        {
            networktest = new NeuralNetwork(numbernodes,layers,attribute,data,location,binary);
