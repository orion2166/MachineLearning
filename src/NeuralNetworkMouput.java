 ...
 */
public class NeuralNetworkMouput {
    ...
    public NeuralNetworkMouput(int nodesNumber, int hiddenLayerNumbers, String attribute_file_location, String database_datasets_filelocatio, int target, boolean convert_binary){
        ...
        if(hiddenLayerNumbers == 0){
            if(convert_binary){
                ...
                outputlayer = new NerualNetworkLayer(max,false, true, value_information.get_attribute_classifications((String) keys.get(0)).size());
                convert_binary_final = true;
            }
            else {
                outputlayer = new NerualNetworkLayer(value_information.get_attribute_size()-1,false,true,value_information.get_attribute_classifications((String) keys.get(0)).size());
            }
            threshold.add(.5);
            return;
        }
        ...
        outputlayer = new NerualNetworkLayer(nodesNumber,false, true, value_information.get_attribute_classifications((String) keys.get(0)).size());
...
//        }
    ...
    boolean test_neural_network(String[] inputvalue){
        ...
        int location = values.get(0).get(inputvalue[0]);
        if(!targets.get(location).equals(tests.get(location)))
                return false;
        return true;
    }
